...
def parse_config(args: List[str]) -> VisionModuleConfiguration:
    ...
    for i in range(len(args)):
        ...
        if len(argument.split("--")) < 2:
            print(ConsoleFormatter.error(f"Vision utilities: Argument in position `{i}` has not the appropiate format: --<name>[=value]"))
            sys.exit(-1)
        ...
        if name not in keys:
            print(ConsoleFormatter.error(f"Vision utilities: Argument `--{name}` is not valid. Check for documentation"))
            sys.exit(-1)
